<?php class cliente{
	...
	public $IdActiveCampaignContact;

	public function buscar($vId,$conexion){
		$SQL = "SELECT * FROM cliente WHERE `id`= '".$vId."'";
		...
		$this->IdActiveCampaignContact=0;
		if( mysqli_num_rows($resultado)!=0 ){
			...
			$this->IdActiveCampaign=$arreglo[42];
			$this->IdActiveCampaignContact=$arreglo[43];
		}else{
	...
	public function grabar($conexion){
		...
		$SQL = "UPDATE cliente SET 
				...
				`IdActiveCampaignContact` = '" .mysqli_real_escape_string($conexion,$this->IdActiveCampaignContact). "' 
				WHERE `id`= '".$this->id."'";
				mysqli_query($conexion,$SQL);
		}
		//echo $SQL;
